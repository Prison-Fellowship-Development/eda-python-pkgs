...
pb2_spec = importlib.util.spec_from_file_location("pubsub_api_pb2", f"{local_dir}/pubsub_api_pb2.py")
pb2 = importlib.util.module_from_spec(pb2_spec)
sys.modules["pubsub_api_pb2"] = pb2
pb2_spec.loader.exec_module(pb2)

pb2_grpc_spec = importlib.util.spec_from_file_location("pubsub_api_pb2_grpc", f"{local_dir}/pubsub_api_pb2_grpc.py")
...
class PubSubAPI:
    ...
    def __init__(self, logger=None):
        ...
        self.sf_channel_host_port = os.getenv('SALESFORCE_CHANNEL_HOST_PORT', 'api.pubsub.salesforce.com:7443')
    ...
    def fetch(self):
        ...
        with grpc.secure_channel(self.sf_channel_host_port, creds) as channel:
            ...
            self.logger.debug("Subscribing to Salesforce topic {}".format(self.sf_topic))
            authmetadata = self.getAuthMetadata()
            substream = stub.Subscribe(
                fetchReqStream(self.semaphore, self.sf_topic, self.sf_replay_preset, self.sf_replay_id)
            ...
            for data in substream:
                ...
                if data.events:
                    ...
                    decoded = None
                    self.logger.info("No data but the Salesforce subscription is still active.")
                    time.sleep(1)
