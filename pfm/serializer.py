...
class Serializer:
    def __init__(self, subject_name, format="AVRO"):
        ...
        self.username_password = (
            os.getenv("PFM_EVENT_SASL_USERNAME", "superuser")
            + ":"
            + os.getenv("PFM_EVENT_SASL_PASSWORD", "secretpassword")
        )
        self.schema_registry_client = SchemaRegistryClient(
            conf={
                "url": os.getenv(
                    "PFM_EVENT_SCHEMA_REGISTRY_URL",
                    "http://localhost:18081,http://localhost:28081,http://localhost:38081",
                ),
